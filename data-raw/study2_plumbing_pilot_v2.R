# Study 2 plumbing pilot, convergence-classification revision
#
...
condition_diagnostics <- function(replicates) {
  ...
  diagnostics <- lapply(condition_ids, function(condition_id) {
    ...
    method_diagnostics <- lapply(methods, function(method) {
      ...
      optimizer_warning_values <- column_or_default(
        method_results,
        "optimizer_warning",
        NA_character_
      )
      optimizer_code_values <- column_or_default(
        method_results,
        "optimizer_code",
        NA_real_
      )
      ...
      data.frame(
        ...
        optimizer_warning_rep_rate = 100 * mean(
          has_text(optimizer_warning_values)
        ),
        nonzero_optimizer_code_rate = if (
          all(is.na(optimizer_code_values))
        ) {
          NA_real_
        } else {
          100 * mean(
            optimizer_code_values != 0,
            na.rm = TRUE
          )
        },
...
metadata <- list(
  study = "mmiCATs Study 2 plumbing pilot, convergence-classification revision",
  purpose = paste(
    "Validate the corrected singularity-versus-convergence",
    "classification across all 24 frozen Study 2 conditions,",
    "checkpointing,",
...
diagnostic_problems <- pilot_diagnostics[
  pilot_diagnostics$failure_rate > 0 |
    pilot_diagnostics$optimizer_warning_rep_rate > 0 |
