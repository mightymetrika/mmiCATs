...
study3c_verify_freeze <- function(project_root) {
  freeze_dir <- file.path(
    project_root, "data-raw", "study3-results", "pre-results-freeze"
  )
  required <- c(
  ...
  study3c_verify_checksum_record(
    file.path(freeze_dir, "study3_source_checksums.csv"),
    project_root
  )
  ...
  if (!identical(mmiCATs:::study2_method_names(), study3c_methods())) {
    stop("Frozen nine-method schedule has changed.", call. = FALSE)
  }

  default_seed <- as.integer(eval(
    formals(mmiCATs::cluster_model_diagnostics)$seed
  ))
  ...
  lookup <- setNames(metadata$value, metadata$field)
  ok <- identical(
...
study3c_prepare_analysis_data <- function(data) {
  x <- mmiCATs:::cluster_diag_prepare_data(
...
study3c_fit_full <- function(
  ...
  mmiCATs::cluster_model_diagnostics(
...
study3c_fit_loo_subject <- function(
  ...
  rows <- lapply(methods, function(method) {
    ...
    data.frame(
      method = method,
      method_label = mmiCATs:::cluster_diag_method_labels(method),
...
study3c_loo_plot <- function(x) {
  ...
  d$method_label <- factor(
    d$method_label,
    levels = mmiCATs:::cluster_diag_method_labels(study3c_methods())
  )
  ggplot2::ggplot(
